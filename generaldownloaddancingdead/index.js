...
const BOARD_ID = process.env.MONDAY_BOARD_ID_DD;
const PLAYLIST_API_URL = "https://amethyst-plume-reading.glitch.me/dancingdeadplaylist";
      ...
      boards(ids: [${BOARD_ID}]) {
        ...
        items_page (limit: 300) {
          ...
          items {
            id 
            name
            column_values (ids: ["texte7","date4", "files", "files_mkmx865m", "status_mkmxtwpj", "texte8"]) {
              id
              text
            }
            assets { 
    ...
    try {
        ...
        if (
            ...
            data.data.boards[0].items_page.items.forEach(item => {
                ...
                const statusColumn = item.column_values.find(column => column.id === "status_mkmxtwpj");
